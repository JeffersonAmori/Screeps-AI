...
export class CreepFactory {
  ...
  private GetBodyPartsInternal(desirableBody: BodyPartRequest[], sortBody: boolean = true): BodyPartConstant[] {
    ...
    let energyAvailable: number = 300; // GlobalMemory.RoomInfo[this._room.name].noActiveResourceHarvest ? 300 : Math.max(this._room.energyAvailable, Math.max(this._room.energyCapacityAvailable / 2, 300));
